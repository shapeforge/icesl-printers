...
-- this version uses only A & B commands in place of E for G0 and G1: this allows simultaneous extrusion on both extruders
...
extruder_e{}    -- table for extrusion length caching for both extruders
extruder_e[0] = 0
extruder_e[1] = 0
...
letter{}    -- extruder table for G commands
letter[0] = 'A'
letter[1] = 'B'
...
function header()
  ...
  if number_of_extruders == 1 then
    ...
    h = h:gsub( '<PRIMINGTOOL0>', 'G1 X220 F2400' .. '; no priming on first move')
    h = h:gsub( '<PRIMINGTOOL1>', 'G1 X35 E10 F2400' .. '; priming on second move')
  ...
  elseif number_of_extruders == 2 then
      ...
      current_extruder = 0
      h = h:gsub( '<PRIMINGTOOL0>', 'G1 X220 B10 F2400 T1' .. '; priming of left extruder')
      current_extruder = 1
      h = h:gsub( '<PRIMINGTOOL1>', 'G1 X35 A10 F2400 T0' .. '; priming on right extruder')
  else
...
function Qretract(speed,len,swapping)
  if current_z > qidi_retract_after_z or swapping then
    extruder_e[current_extruder] = extruder_e[current_extruder] - len
    letter = letter[current_extruder]
    output('G1 F' .. speed .. ' ' .. letter .. ff(extruder_e[current_extruder] - last_e))
  end
   return extruder_e[current_extruder]
end

function Qrestore(speed,len,swapping)
  if current_z > qidi_retract_after_z or swapping then
    extruder_e[current_extruder] = extruder_e[current_extruder] + len  + extruder_e_restart
    letter = letter[current_extruder]
    output('G1 F' .. speed .. ' ' .. letter .. ff(extruder_e[current_extruder] - last_e))
  end
   return extruder_e[current_extruder]
...
function move_xyz(x,y,z)
  -- OLD CODE for Z caching only
  -- if z == current_z and z_caching then
  --   output('G1 X' .. f(x) .. ' Y' .. f(y))
  -- else
  --   current_z = z
  --   output('G1 X' .. f(x) .. ' Y' .. f(y) .. ' Z' .. f(z * z_ratio + qidi_z_offset))
  -- end
...
function move_xyze(x,y,z,e)
  extruder_e[current_extruder] = e
      ...
      -- OLD CODE for Z caching only
  -- if z == current_z and z_caching then
  --   output('G1 X' .. f(x) .. ' Y' .. f(y) .. ' ' .. letter .. ff(extruder_e[current_extruder] - last_e))
  -- else
  --   current_z = z
  --   output('G1 X' .. f(x) .. ' Y' .. f(y) .. ' Z' .. f(z * z_ratio + qidi_z_offset) .. ' ' .. letter .. ff(extruder_e[current_extruder] - last_e))
  -- end
  ...
  output('G1' .. x_string .. y_string .. z_string .. ' E' .. ff(extruder_e[current_extruder] - last_e))
end

function move_e(e)
  extruder_e[current_extruder] = e
  letter = letter[current_extruder]
  output('G1' .. ' ' .. letter .. ff(extruder_e[current_extruder] - last_e))
end
